...
- <a id="SPEC-08"></a>**[SPEC-08]** Mistralの応答は以下のスタイルを必ず満たす *(要件: [REQ-04](./requirements.md#REQ-04), [REQ-05](./requirements.md#REQ-05))*
  ...
  - スタート地点・本当の目的を振り返らせる
  - 完璧主義やデジタル依存を優しく批判する
  - Doomer的思考に対して「終末は勝手に来る。望むな、備えろ」と返す
  - 最終アウトプットは手書きであるべきだと繰り返し示唆する
- <a id="SPEC-09"></a>**[SPEC-09]** 応答文は声で自然に読み上げられる長さ（短文中心）とする *(要件: [REQ-06](./requirements.md#REQ-06))*
...
- <a id="SPEC-18"></a>**[SPEC-18]** 再開後もターンカウントをリセットせず、セッション全体で7ターン制限を維持する（必要に応じて別セッション化の設計余地を残す） *(要件: [REQ-19](./requirements.md#REQ-19), [REQ-20](./requirements.md#REQ-20))*
